...
var GetCmd = &cobra.Command{
	Use:   "get",
	Short: "Get details of a specific report",
	Long: `Get details of a specific report by ID. Takes report ID from stdin. Example:

echo "report_id_here" | acucli report get`,
	Run: func(cmd *cobra.Command, args []string) {
		input := filehelper.ReadStdin()
		if input == nil || len(input) == 0 {
			jsonoutput.OutputErrorAsJSON(fmt.Errorf("no report ID provided"), "Error")
			return
		}
